...
def sample_initial_conditions(model, points_to_sample, random_state, traj_length=1000, pts_per_period=30):
    ...
    sample_inds = rand.choice(np.arange(initial_sol.shape[0]), points_to_sample, replace=False)
...
def _make_data(equation_name, dt, n, seed_list, noise_list):
    ...
    for noise_ratio in noise_list:
        ...
        for seed in seed_list:
            ...
            if len(seed_list) == 1:
                data_each_seed = sol + noise * (data_norm * noise_ratio) / noise_norm
            else:
            ...
            for k, feature in enumerate(np.flip(feature_names[1:])):
                ...
                if feature_ind != -1:
                    ...
                    if feature_chunk != chunk:
                        ...
                        if len(feature_chunk_compact[: -len(feature) - 1].replace(' ', '')) == 0:
...
class Data:
    def __init__(self):
        ...
        self.true_coefficients = make_dysts_true_coefficients(systems_list, dimension_list, self.param_list)
    ...
    def make_all_data(self, noise_list, seed_list, dt=0.01, n=500):
        ...
        with ProcessPoolExecutor(max_workers=num_works, mp_context=multiprocessing.get_context('spawn')) as executor:
            futures = {
                executor.submit(_make_data, equation_name, dt, n, seed_list, noise_list): equation_name
                for equation_name in self.systems_list
            }
            i = 0
            data_list = {}
    ...
    def make_each_data(self, equation_name, noise_list=[0.05], seed_list=[1], dt=0.01, n=500):
        self.dt = dt
        self.n = n
...
if __name__ == '__main__':
    dysts = Data()
    for name in dysts.systems_list:
        data = dysts.make_each_data(equation_name=name, noise_list=NOISE_LIST, dt=DT, n=LENGTH)
        for noise in NOISE_LIST:
            _data = data[noise]
            df = pd.DataFrame(_data, columns=[f'x{i + 1}' for i in range(_data.shape[1])])
            df['date'] = pd.date_range('2000-01-01', periods=_data.shape[0], freq='s')
            cols = df.columns.tolist()
            cols = cols[-1:] + cols[:-1]
            df = df[cols]

            output_path = f'data/dysts/raw_data/{name}.csv.gz'
